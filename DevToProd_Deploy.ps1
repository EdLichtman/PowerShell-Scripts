...
Else {
	...
	$lastBackup = Get-ChildItem backups | Where-Object {$_.name -like "bin$(get-date -f yyyyMMdd)*"} | sort name -Descending | select -First 1
	...
	Copy-Item .\bin\* .\$newBackupName\bin

	If ((Get-ChildItem | Where-Object {$_.name -eq "Web.config"})){
	Copy-Item .\Web.config .\$newBackupName\}
